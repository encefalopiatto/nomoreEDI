...
- **[DECISION.md](DECISION.md)** — the build recommendation: four products in strict order (contract file → compiler → conformance checkmark → change ledger, language last), the concrete engineering plan for each, and a plain-language version of the whole plan for non-developers (Part III).
- **[SUPERMESSAGE.md](SUPERMESSAGE.md)** — the supermessage specification v0.1, in plain language: one file carrying the business message plus its complete rulebook, response instructions, connection details, and answer key — the traveling form of the rulebook. Worked example: [spec/example-order.supermessage.json](spec/example-order.supermessage.json).
- **[NODE.md](NODE.md)** — the exchange node prototype: one small program per company that receives, checks, answers, and routes supermessages, treats incoming files as configuration proposals, and applies nothing without human review. Comes with a guided two-company demo you can run on your own machine. Code in [exchange-node/](exchange-node/).
